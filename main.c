...
int main() {
    ...
    if (numRooms > MAX_ROOMS || numRooms < 1) {
        printf("Invalid number of rooms! Exiting...\n");
        return 1;
    }

    initializeSystem(numRooms, lights, temperatures, motion, locks);
...
void analyzeHouseStatus(int numRooms, int lights[], int temperatures[], int motion[], int locks[]) {
    printf("House Status:\n");
    for (int i = 0; i < numRooms; i++) {
        printf("- Room %d: Light %s, Temperature: %d°C, Motion %s, Door %s\n",
               i + 1, lights[i] ? "ON" : "OFF", temperatures[i],
               motion[i] ? "Detected" : "No Motion", locks[i] ? "Locked" : "Unlocked");
    }
}
